...
const wreiskeIonic4PackageName = 'wreiske-ionic4';
...
Package.describe({
	...
	git: 'https://github.com/wreiske/meteor-ionic4.git',
...
Package.onUse(api => {
	...
	const wreiskeIonic4PackagePath = isMeteorProject('.') ?
		getWReiskeIonic4Package('.') :
		'.';

	const distPath = wreiskeIonic4PackagePath ?
		path.join(wreiskeIonic4PackagePath, 'dist') :
		null;
	const files = distPath ? getFilesRecursive(distPath) : null;
	if (files) {
		const fileRelPathes = files.map(p => {
			return path.relative(wreiskeIonic4PackagePath, p);
		});
...
function getWReiskeIonic4Package(source) {
	...
	return packagePathes.find(p => {
		...
		const result = isWReiskeIonic4PackageJson(packageJsonPath);
		return result;
	});
}

function isWReiskeIonic4PackageJson(source) {
	if (!isFile(source)) {
		return false;
	}
	...
	return packageName === wreiskeIonic4PackageName;
}
